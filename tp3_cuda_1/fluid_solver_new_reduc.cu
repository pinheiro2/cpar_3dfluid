...
float gpu_reduce(float *d_input, int n)
{
  float *d_intermediate, *d_output;
  int threads = 256;
  int blocks = (n + threads - 1) / threads;

  // Allocate intermediate storage for the reduction results
  cudaMalloc(&d_intermediate, sizeof(float) * blocks);
  cudaMalloc(&d_output, sizeof(float));

  // Perform reductions until the result fits into one block
  while (blocks > 1)
  {
    reduction_kernel<<<blocks, threads, threads * sizeof(float)>>>(d_input, d_intermediate, n);
    cudaDeviceSynchronize();

    // Prepare for the next level of reduction
    n = blocks;
    blocks = (n + threads - 1) / threads;

    // Swap input and intermediate
    float *temp = d_input;
  ...
  // Final reduction
  reduction_kernel<<<1, threads, threads * sizeof(float)>>>(d_input, d_output, n);
  cudaDeviceSynchronize();
...
void lin_solve(int M, int N, int O, int b, float *x, float *x0, float a, float c)
{
  ...
  // Allocate memory for max_changes on the device
  cudaMalloc((void **)&d_max_changes, size);
  ...
  while (l < max_iterations)
  {
    ...
    // Perform full reduction on GPU to get max_c
    float max_c = gpu_reduce(d_max_changes, M * N * O);
  ...
  // Free memory for max_changes
  cudaFree(d_max_changes);
}

void lin_solve_recHost(int M, int N, int O, int b, float *x, float *x0, float a, float c)
{
...
void advect(int M, int N, int O, int b, float *d, float *d0, float *u, float *v, float *w, float dt)
{
  ...
  dim3 threadsPerBlock(16, 16, 4); // Adjust as necessary for hardware
  dim3 numBlocks((M + threadsPerBlock.x - 1) / threadsPerBlock.x,
                 (N + threadsPerBlock.y - 1) / threadsPerBlock.y,
                 (O + threadsPerBlock.z - 1) / threadsPerBlock.z);
