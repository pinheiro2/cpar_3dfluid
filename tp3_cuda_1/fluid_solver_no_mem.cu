...
__global__ void reduction_kernel(float *max_changes, float *result, int size)
{
  ...
  shared_data[tid] = (index < size) ? max_changes[index] : 0.0f;
  __syncthreads();
...
void lin_solve(int M, int N, int O, int b, float *x, float *x0, float a, float c)
{
  ...
  cudaMalloc((void **)&d_max_result, sizeof(float) * reduction_size);
  ...
  while (l < max_iterations)
  {
    ...
    int blocks = (M * N * O + 255) / 256;
    reduction_kernel<<<blocks, 256, 256 * sizeof(float)>>>(d_max_changes, d_max_result, M * N * O);
    cudaDeviceSynchronize();

    // Copy the result back to host
    float *max_result = new float[blocks];
